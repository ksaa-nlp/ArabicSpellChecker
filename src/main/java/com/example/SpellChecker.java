...
import java.io.*;
import java.util.*;
import static com.example.ArabicUtils.*;
...
public class SpellChecker {
    ...
    public static void main(String[] args) throws IOException {
        ...
        while (i < args.length) {
            arg = args[i++];
            //
            if (arg.equals("--help") || arg.equals("-h") || (args.length != 0 && args.length != 2 && args.length != 4
                    && args.length != 6 && args.length != 8)) {
                System.out.println(
                        "Usage: SpellChecker <--help|-h> <[-i|--input] [in-filename]> <[-o|--output] [out-filename]>");
                System.exit(-1);
            }
        ...
        // input = "وبحضاراته الشسيعة فقط,";// "يتحدثهاأكثر بالحوارفعليك وضعتهاايران";//
        // "الارهاربيةتنتظر"; //"مليوننسمة ثلاتة";
        // input = "النصر آت لا محال انشاء الله من يؤمن بالله واليوم الآخر . والمطلع على
        // قصص الانبياء يرى ان النصر من الله وعلامات ألقيامه آتيه في تسلسل عجيب . إن
        // تنصر الله ينصركم لا رجوع انما التغيير آت ليس من الرءساء أنما من فوق سابع
        // سماء";
    ...
    public int init(boolean considerContext) {
        ...
        try {
            if (!readFromSerFiles) {
                ...
                // candidatesUnigram = loadCandidates(dir +
                // "mgb.arabic.unnormalized.spellDict"); //aj.lm.words.uni.trim
        ...
        // msg = String.format("init() end. candidatesUnigram.size()=%d",
        // candidatesUnigram.size());
        // System.out.println(msg);
    ...
    private HashMap<String, String> loadCandidates(String filePath)
        ...
        if (resolveName(filePath + ".ser") != null) {
            ...
            // nofLines = 0;
            // BufferedReader sr = new BufferedReader(new InputStreamReader(new
    ...
    private HashMap<String, String> loadCorrections(String filePath)
            throws FileNotFoundException, IOException, ClassNotFoundException {
        int i, nofLines;
        String msg, line = "";
        List<String> fileLines = new ArrayList<String>();
    ...
    private HashMap<String, String> loadPunctFreq(String filePath)
            throws FileNotFoundException, IOException, ClassNotFoundException {
        int i, nofLines;
        String msg, line = "";
        List<String> fileLines = new ArrayList<String>();
    ...
    private HashMap<String, Integer> loadNGrams(String filePath, int ngram)
            throws FileNotFoundException, IOException, ClassNotFoundException {
        int i, nofLines, freq;
        String msg, line = "";
        List<String> fileLines = new ArrayList<String>();
        ...
        if (resolveName(filePath + ".ser") != null) {
            ...
            for (i = 0; i < nofLines; i++) {
                if ((i % 10000) == 0) {
                    msg = String.format("   (). lineNo=%d/%d", i, nofLines);
                    System.out.println(msg);
                }
    ...
    private HashMap<String, String> loadAljazeeraBW(String filePath)
            throws FileNotFoundException, IOException, ClassNotFoundException {
        int i, nofLines;
        String msg, value;
        List<String> fileLines = new ArrayList<String>();
    ...
    /*
     * private static HashMap<String, Double> loadKenLM(String filePath) throws
     ...
     * {
     * String[] lineParts = line.split("\t");
     * if (line.length() > 0 && lineParts.length > 1) // &&
     * Regex.IsMatch("^[0-9\\.\\-]$"))
     * {
    ...
    public String spellCheck(String input, boolean formatInput, boolean formatOutput, boolean considerContext) {
        ...
        // For site display, show formatted output and try all edits: insertion,
        ...
        try {
            ...
            for (i = 0; i < nofLines; i++) {
                ...
                System.err.println(lattice.size());
    ...
    private HashMap<Integer, ArrayList<String>> buildLattice(ArrayList<String> words,
            ArrayList<Integer> latticeIndexes) {
        int i = 0, j, k, m, len, orgWordIndex, nofWords, freq0, freq1, maxFreq, orgWordFreq, splitAction, tmpFreq,
                breakpoint, oov, latticeIndex;
        ...
        for (j = 0; j < nofWords; j++) {
            ...
            if (corr.length() > 0) {
                ...
                if (nofCandidates[0] == 0) {
                    ...
                    if (nofCandidates[0] > 0) {
                        // Try to correct word before full normalization, ex: شئ ->شيء before converting
                        // it to شء
                        ...
                        // If the word has only one form at unigram file, and has no error, then keep
                        // the original word, ex: إعتقاد
                        if (nofCandidates[0] == 1) {
                            String[] wordFreq = candidates.split("_");

                            if (orgCAM.length() > 0) {
                                // Word الطائفه has only one correct form "الظائفة" and this is the same like
                                // CAM correction. So, take any one of them
                        ...
                        // Word doesn't appear in unigrams (OOV: may be a NE or a normal word that is
                        // not exist in LM)
                        breakpoint = 1;
                    }
                }
            ...
            if (nofCandidates[0] > 1) {
                ...
                // Solve conflict between candidate unigrams and splitting errors: ماهذا ماهذا
                // ماهذأ
                // Get best frequency for candidate unigrams and splitting, then select the
                // correct action
                splitAction = 0;
                ...
                if (splitAction == 0) {
                    // Hamdy, 2014-06-26. Filter candidates according to aljazeear unigrams
                    // frequency. ايضا أيضا ايضا إيضا آيضا أيضأ ايضأ أىضا إيضأ
                    k = 0;
                    freq0 = 0;
                    maxFreq = 0;
                    orgWordFreq = 0;
                    ...
                    if (temp.size() > 0) {
                        // Solve errors in candidate unigrams, الفاءده الفاءدة الفاءده, قاريء قارىء
                        // قاريء
                        if (freq0 <= 1) {
                ...
                // Correct words that are found in aljazeera but not found in candidate
                // unigrams, ex: اباحت، اباحوا
    ...
    // private static String getPunct(String w, int j, int nofWords,
    // ArrayList<String> words, int[] offset) {
    ...
    private String correctSurfaceErrors(String s) {
        ...
        // Sometimes, words at aljazeera corpus contain errors, ex: اللذي. So, give
        ...
        if ((freq0 == 0) && (freq1 == -1)) {
            // Orgiginal word is not found in the aljazeera words and it's found in
            // candidate unigrams for corrections
            breakpoint = 1;
            ...
            if ((freq0 == 1) && (freq1 >= 10)) {
                // Word has very low frequency = 1 (تقفو) and its correction (تقفوا) has a
                // better frequency = 40
                breakpoint = 1;
            } else {
                corr = "";
    ...
    private String correctCAM(String s, int maxNofSugg, int[] bestFreqCAM) {
        ...
        if (len > 1) {
            for (i = 0; i < len; i++) {
                ...
                if ((i == len - 3) && ((ch == 'ي') || (ch == 'ى')) && (s.charAt(i + 1) == 'ى')
                        && (s.charAt(i + 2) == 'ء')) {
                    // شيىء -> شيء
    ...
    private String correctUnknownWords(String w) {
        ...
        boolean searchInCandidateUnigrams = true;
        // String[] prefixes = {"و", "ف", "ال", "ب", "ل", "ك", "وال", "فال", "وب", "ول",
        // "فب", "فل", "وبال", "ولل", "فبال", "فلل", "س"};
        String[] prefixes = { "و", "ف", "ب", "ل", "ك", "س" };
        String[] anit = { "أ", "ن", "ي", "ت" };
        // String[] suffixes = {"ي", "نا", "ك", "كما", "كم", "كن", "ه", "ها", "هما",
        // "هم", "هن", "ت", "تما", "تم", "تن", "ا", "تا", "وا", "ن", "ين", "ي", "ون",
        // "ان",
    ...
    private String splitWord(String s, String prev, String next) {
        ...
        if (len > 4) {
            for (i = 1; i < len - 2; i++) {
                ...
                trySplit = false;
                commonSplit = false;
                if ((ch == 'ا') || (ch == 'ء') || (ch == 'ة') || (ch == 'د') || (ch == 'ذ') || (ch == 'ر')
                        || (ch == 'ز') || (ch == 'و') || (ch == 'ى')) {
                    commonSplit = true;
                } else if ((len > 8) && (i >= 3) && (i < len - 4)) {
                    // Add more restrictions on splitting. Try splitting to 2 parts with length >= 4
                    // characters
                    trySplit = true;
                }
                ...
                if (commonSplit || trySplit) // FIXME: true
                {
                    ...
                    if (commonSplit && (score0 == 0) && (score1 == 0)) {
                        ...
                        if (corr1.equals("لا") || corr1.equals("يا") || corr1.equals("أو") || corr1.endsWith("ة")
                                || corr1.endsWith("اء") || corr2.startsWith("ال")) {
                    ...
                    // sometimes, scores are close to each others (قادرون/قاد رون), so use
                    // percentage instead of comparison
                    if (score1 > score0) {
                        if (score1 > bestScore) {
                            bestScore = score1;
    ...
    private String findBestPath(HashMap<Integer, ArrayList<String>> lattice, ArrayList<String> words,
            ArrayList<Integer> latticeIndexes, boolean formatOutput) throws IOException {
        boolean breakpoint;
        int i, nofTokens, len, punctTotafFreq, punctBeforeFreq, punctAfterFreq, j, m, n, c, latticeSize, wordIndex,
                wordIndex2, nofInputWords, freq, freq2, maxFreq;
        int[] nofCandidates = new int[1];
        String space = " +", sBase, tmp, w, w2, punct, punctBefore, punctAfter, correction, undiacWord, undiacWord2,
                aljazeeraBW, candidates, norm;
            ...
            if (bestPath.length == i + 2) {
                if (formatOutput) {
                    ...
                    if (!undiacWord2.equals(undiacWord)) {
                        ...
                        // FIXME: favor use input if it's a high frequent word, ex:اليأس is corrected to
                        // إلياس
                        /*
    ...
    private boolean wordHasPunct(String w) {
        ...
        if (w.length() == 1) {
            punct = true;
        } else if (w.contains("،") || w.contains(".") || w.contains("-") || w.contains("!") || w.contains("؟")
    ...
    public ArrayList<String> edits(String word, int maxNofSugg) {
        ...
        // final String[] SUFFIXES = {"ة", "ه", "ها", "هما", "هم", "هن", "ك", "كما",
        // "كم", "كن", "ون", "ين", "وا", "و", "ي", "ان", "ات", "ا"};
        final String[] SUFFIXES = { "ا", "ة", "ت", "ك", "ن", "و", "ي", "ات", "ان", "ون", "وا", "ين", "كما", "كم", "كن",
                "ه", "ها", "هما", "هم", "هن", "نا", "تما", "تم", "تن" };
